...
public class Tree {
	...
	public static Tree play_board(Tree tree_root, ArrayList<ArrayList<String>> board_points, String x){
		int utility_value;
		Tree child;
		//		play(tree_root);
		for ( Tree each : tree_root.getChildren() ){
			utility_value = calculate_utility(board_points, each.getId());
			System.out.println(x + x + "Hello" + " "+ utility_value + each.getId());
			play_board(each, board_points, x + x);
			//System.out.println(each.getId());
			
		}
		
//		child = (Tree) tree_root.getChildren();
